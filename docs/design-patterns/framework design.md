...
架构是有关软件 **整体结构与组件的抽象描述**，用于指导大型软件系统各个方面的设计。可以看出框架和架构都和组件有关系，但架构关注的是“结构”，框架关注的是“规范”。

架构是一个介于需求与实现中间的步骤，既要分析需求，又要可以提出可行的实现方案，是软件系统的顶层结构。
